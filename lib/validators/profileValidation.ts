...
export function validatePassword(password: string): string | null {
  ...
  if (!password || password.trim() === "") {
    return UXCopy.validation.passwordRequired;
  }

  // 2. 길이 체크 (8~20자)
  if (password.length < 8 || password.length > 20) {
    return UXCopy.validation.password.length;
  }

  // 3. 영문 대문자 포함
  if (!/[A-Z]/.test(password)) {
    return UXCopy.validation.password.needUpper;
  }

  // 4. 영문 소문자 포함
  if (!/[a-z]/.test(password)) {
    return UXCopy.validation.password.needLower;
  }

  // 5. 숫자 포함
  if (!/\d/.test(password)) {
    return UXCopy.validation.password.needNumber;
  }

  // 6. 특수문자 포함
  if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password)) {
    return UXCopy.validation.password.needSpecial;
  }

  // 7. 공백 금지
  if (/\s/.test(password)) {
    return UXCopy.validation.password.noWhitespace;
  }
