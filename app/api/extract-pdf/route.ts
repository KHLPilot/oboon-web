...
import {
  buildCompanyRescueText,
  buildPhase1ExtractionText,
  buildSpecsSupplementText,
  buildTimelineSupplementText,
  buildUnitSupplementText,
  buildValidationSupplementText,
  buildWebRefinementText,
  systemPrompt,
} from './prompts';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    > = [
      {
        type: 'text',
        text: buildPhase1ExtractionText(extractionMode, inputFiles.length, extractedText),
      },
    ];
    ...
    if (companyRescueNeeded && hasRemainingTime(requestStartedAt, 40000)) {
      ...
      > = [
        {
          type: 'text',
          text: buildCompanyRescueText(extractionMode, extractedText),
        },
      ];
    ...
    if (
      ...
      > = [
        {
          type: 'text',
          text: buildSpecsSupplementText(extractedText, extractionResult.specs),
        },
      ];
    ...
    if (
      ...
      > = [
        {
          type: 'text',
          text: buildTimelineSupplementText(extractedText, extractionResult.timeline),
        },
      ];
      for (const image of tableCandidateImages) {
    ...
    if (
      ...
      > = [
        {
          type: 'text',
          text: buildValidationSupplementText(extractedText, extractionResult.validation),
        },
      ];
      for (const image of tableCandidateImages) {
    ...
    if (
      ...
      > = [
        {
          type: 'text',
          text: buildUnitSupplementText(extractedText, extractionResult.unit_types),
        },
      ];
      for (const image of tableCandidateImages) {
