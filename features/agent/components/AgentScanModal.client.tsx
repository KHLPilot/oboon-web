...
import {
  fetchAgentScanBootstrap,
  subscribeToVisitConfirmRequests,
} from "@/features/agent/services/agent.scan";
...
type AgentScanContentProps = {
  initialConsultationId?: string | null;
  embedded?: boolean;
};

export function AgentScanContent({
  initialConsultationId = null,
  embedded = false,
}: AgentScanContentProps) {
      ...
      try {
        ...
        if (
          ...
          data.consultations?.some(
            (c: Consultation) => c.id === initialConsultationId,
          )
        ) {
  ...
  async function generateToken(consultationId?: string) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showAlert(data.error || "QR 토큰 생성에 실패했습니다");
      }
    } catch (err) {
      console.error("QR 토큰 생성 오류:", err);
      showAlert("QR 토큰 생성 중 오류가 발생했습니다");
    } finally {
      setGenerating(false);
    }
  }
  ...
  return (
    <Wrapper>
      <div className="max-w-lg mx-auto space-y-6">
        <div>
          <div className="text-2xl font-bold text-(--oboon-text-title)">
            방문 인증 QR
          </div>
...
export default function AgentScanModal({
  open,
  consultationId,
  onClose,
}: AgentScanModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} size="lg" panelClassName="p-0">
      <div className="px-6 pb-6 pt-4 max-h-[80vh] overflow-y-auto">
        <AgentScanContent embedded initialConsultationId={consultationId} />
      </div>
    </Modal>
  );
}
